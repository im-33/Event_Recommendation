...
public class RpcHelperTest {
	...
	public void testGetJSONArrayCornerCases() throws JSONException {
		...
		Item one = new ItemBuilder().setItemId("one").setDistance(33.33).setRating(5).setCategories(category).build();
		Item two = new ItemBuilder().setItemId("two").setDistance(33.33).setRating(5).setCategories(category).build();
		listItem.add(one);
		listItem.add(two);
